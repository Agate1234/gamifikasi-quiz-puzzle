  ...
  try {
    ...
    for (const item of jawaban) {
      if (!item.jawaban_soal || typeof item.is_true !== "boolean") {
        return res.status(400).json({
          success: false,
          message: "Setiap jawaban harus punya jawaban_soal dan is_true boolean",
        });
      }
    }
    ...
    if ((tipe_soal === "pilgan" || tipe_soal === "true_false") && jumlahBenar !== 1) {
      return res.status(400).json({
        success: false,
    ...
    return res.status(201).json({
      success: true,
      message: "Soal dan jawaban berhasil ditambahkan",
      data: {
        soal: soalBaru,
